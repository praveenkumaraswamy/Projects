...
/*$sql = "INSERT INTO transaction (DueDate,MemberId,ReturnDate,BookNo) VALUES (DATE_ADD(CURRENT_DATE, INTERVAL 15 DAY),".$_SESSION['gv_memberid'].",DATE_ADD(CURRENT_DATE, INTERVAL 15 DAY),'$BookNo') ";
...
if (mysqli_query($conn, $sql3)) {
  ...
  $sql5 = "UPDATE transaction set ReturnDate = CURRENT_DATE WHERE BookNo='$BookNo'";

  $result5 = mysqli_query($conn,$sql5);

  if($result5){  echo '<div class="alert alert-success alert-dismissible fade show" role="alert ">
    <strong>Successfully UPDATED !</strong> You should check in Database.
    <button type="button" onclick="clicking()" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
  </div>';
  echo "<br>";
    ...
    echo "Error in return date";
  }


  echo 'SUCCESSFULLY RETURN THE BOOK HAVE A NICE DAY';
  echo '<br>';
  echo '<button type="button" onclick="redirect()" class="btn btn-danger mt-4 ml-4">Home</button>';
  echo '</div>';
